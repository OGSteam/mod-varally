...
if ($tag == '') {
    echo '<table width=\'100%\'><tr><td class=\'c\'>Aucune alliance Configurée</th></tr></table></br>';
} else {
    $whereDate = '';
    ...
    foreach ($listTag as $tag) {
        ...
        switch ($nb) {
            ...
            case '1':
                ...
                $evolution =
                    'Classement du ' . date('d/m/Y H:i:s', $new);
...
?>
            ...
            <form method='post' action='?action=varAlly&subaction=ally#bilanJ<?php echo $tag ?>'
                  name="bilanJ<?php echo $tag ?>" id="bilanJ<?php echo $tag ?>">
                <fieldset>
                    Nombre de joueurs à afficher pour&nbsp;:<br/>
                    Les plus fortes évolutions <input type="text" name="evol<?php echo $tag ?>"
                                                      value="<?php if (isset(${'pub_evol' . $tag})) echo ${'pub_evol' . $tag}; else echo $nbrjou; ?>">
                    <em>(mettre 0 pour ne pas afficher)</em><br/>
                    Les plus gros gains de points <input type="text" name="pointsplus<?php echo $tag ?>"
                                                         value="<?php if (isset(${'pub_pointsplus' . $tag})) echo ${'pub_pointsplus' . $tag}; else echo $nbrjou; ?>">
                    <em>(mettre 0 pour ne pas afficher)</em><br/>
                    Les chutes <input type="text" name="chute<?php echo $tag ?>"
                                      value="<?php if (isset(${'pub_chute' . $tag})) echo ${'pub_chute' . $tag}; else echo '-1'; ?>">
                    <em>(pour afficher toutes les chutes mettre -1, 0 pour ne pas afficher)</em><br/>
                    Les plus faible gains de points <input type="text" name="pointsmoins<?php echo $tag ?>"
                                                           value="<?php if (isset(${'pub_pointsmoins' . $tag})) echo ${'pub_pointsmoins' . $tag}; else echo $nbrjou; ?>">
                    <em>(mettre 0 pour ne pas afficher)</em><br/><br/>
                    Taille du champs du bilan <input type="text" name="tarea<?php echo $tag ?>"
                                                     value="<?php if (isset(${'pub_tarea' . $tag})) echo ${'pub_tarea' . $tag}; else echo '20'; ?>"><br/>
                    <input type="hidden" name='dateMin' value="<?php echo isset($pub_dateMin) ? $pub_dateMin : $ex; ?>">
                    <input type="hidden" name='dateMax'
                           value="<?php echo isset($pub_dateMin) ? $pub_dateMax : $new; ?>">
                    <input type='submit' value='Afficher'>
                </fieldset>
            </form>
<?php
...
<p>MarkDown (Discord)</p>

<textarea rows="<?php echo ${'pub_tarea' . $tag}; ?>">
Evolution entre le **<?php echo isset($pub_dateMin) ? $pub_dateMin : date('d/m/Y H:i:s', $ex) ?>** et le **<?php echo isset($pub_dateMax) ? $pub_dateMax : date('d/m/Y H:i:s', $new) ?> [<?php echo $tag; ?>]**
<?php
            array_multisort($prc, SORT_DESC, $pts, SORT_ASC, $tblecart);
            if (!isset(${'pub_evol' . $tag})) ${'pub_evol' . $tag} = $nbrjou;

            if (${'pub_evol' . $tag} > 0) {
                echo "\n__Les " . ${'pub_evol' . $tag} . " plus fortes évolutions :__\n";
                for ($a = 0; $a < ${'pub_evol' . $tag}; $a++) {
                    echo "* **" . $tblecart[$a]['joueur'] . "** avec +" . number_format($tblecart[$a]['pts'], 0, '', '.') . " points soit +" . $tblecart[$a]['prc'] . "%\n";
                }
            }
            ...
            if (${'pub_pointsplus' . $tag} > 0) {
                echo "\n__Les " . ${'pub_pointsplus' . $tag} . " plus gros gains de points : __\n";
                for ($b = 0; $b < ${'pub_pointsplus' . $tag}; $b++) {
                    echo "* **" . $tblecart[$b]['joueur'] . "** avec +" . $tblecart[$b]['prc'] . "% soit +" . number_format($tblecart[$b]['pts'], 0, '', '.') . " points\n";
                }
            }
            ...
            if (${'pub_chute' . $tag} != 0) {
                ...
                for ($c = 0; $c < ${'pub_chute' . $tag}; $c++) {
                    $infc = (integer)$tblecart[$c]['pts'];
                    if (strstr($infc, '-')) {
                        echo "* **" . $tblecart[$c]['joueur'] . "** avec " . number_format($tblecart[$c]['pts'], 0, '', '.') . " points soit " . $tblecart[$c]['prc'] . "%\n";
                        $nbr++;
                    }
                }
                if ($nbr < 1) {
                    echo "**Personne n'a chuté**\n";
                }
            }
            ...
            if (${'pub_pointsmoins' . $tag} > 0) {
                echo "\n__Les " . ${'pub_pointsmoins' . $tag} . " plus faible gains de points (faut se reveiller la !!!) : __\n";
                for ($d = 0; $d < ${'pub_pointsmoins' . $tag}; $d++) {
                    $infd = (integer)$tblecart[$d]['pts'];
                    if (!strstr($infd, '-')) {
                        echo "* **" . $tblecart[$d]['joueur'] . "** avec +" . $tblecart[$d]['prc'] . "% soit +" . number_format($tblecart[$d]['pts'], 0, '', '.') . " points\n";
                    } else {
                        ${'pub_pointsmoins' . $tag} = ${'pub_pointsmoins' . $tag} + 1;
                    }
                }
